namespace BossMod.Dawntrail.Foray.ForkedTower.FTB4Magitaur;
...
class RuneAxe(BossModule module) : Components.GenericAOEs(module)
{
    ...
    public override IEnumerable<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        {
            if (bit == 0)
                yield return new AOEInstance(FTB4Magitaur.NotPlatforms, Arena.Center, default, Spreads[0].Activation);
            else
                yield return new AOEInstance(new AOEShapeRect(10, 10, 10), Arena.Center + FTB4Magitaur.Platforms[bit - 1].Item1, FTB4Magitaur.Platforms[bit - 1].Item2, Spreads[0].Activation);
        }
    }
    ...
    public override void Update()
    {
        ...
        for (var i = 0; i < NumActive; i++)
        {
            ...
            foreach (var (offset, angle) in FTB4Magitaur.Platforms)
            {
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        foreach (var s in ActiveSpreads)
        {
            ...
            if (s.Target == pc && s.Radius == 11)
                foreach (var p in FTB4Magitaur.Platforms)
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (ActiveSpreadOn(actor) is { } spread)
        {
            if (spread.Radius == 11)
            {
                foreach (var (off, dir) in FTB4Magitaur.Platforms)
                    // TODO(SDF)
