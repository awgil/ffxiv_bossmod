...
class HolyLance(BossModule module) : Components.GenericAOEs(module, AID.LanceAppear)
{
    ...
    public override IEnumerable<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (NumCasts % 4 == 0)
            yield return new(FTB4Magitaur.NotPlatforms, Arena.Center, Activation: _activation);
        else
        {
            var platform = FTB4Magitaur.Platforms[NumCasts < 4 ? 2 : NumCasts < 8 ? 1 : 0];
...
class PreyLancepoint(BossModule module) : BossComponent(module)
{
    ...
    public override void OnStatusGain(Actor actor, in ActorStatus status)
    {
        if ((SID)status.ID == SID.PreyLancepoint)
        {
            ...
            var platform = FTB4Magitaur.GetPlatform(actor.Position);
