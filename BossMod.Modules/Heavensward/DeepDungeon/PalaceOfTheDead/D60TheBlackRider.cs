...
class HallOfSorrow(BossModule module) : Components.VoidzoneAtCastTarget(module, 9, AID.HallOfSorrow, m => m.Enemies(OID.Voidzone).Where(z => z.EventState != 7), 1.3f);
...
class ValfodrKB(BossModule module) : Components.Knockback(module, AID.Valfodr, stopAtWall: true) // note actual knockback is delayed by upto 1.2s in replay
{
    ...
    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos) => GetFireballZone() is { } z && z(pos) < 0;
...
[ModuleInfo(Contributors = "legendoficeman, Malediktus", GroupType = BossModuleInfo.GroupType.CFC, GroupID = 204, NameID = 5309)]
