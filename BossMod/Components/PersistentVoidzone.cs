...
public class PersistentVoidzone(BossModule module, float radius, Func<IEnumerable<Actor>> sources) : GenericAOEs(module, default, "GTFO from voidzone!")
{
    public AOEShapeCircle Shape { get; init; } = new(radius);
    public Func<IEnumerable<Actor>> Sources { get; init; } = sources;
...
public class PersistentVoidzoneAtCastTarget(BossModule module, float radius, ActionID aid, Func<IEnumerable<Actor>> sources, float castEventToSpawn) : GenericAOEs(module, aid, "GTFO from voidzone!")
{
    public AOEShapeCircle Shape { get; init; } = new(radius);
    public Func<IEnumerable<Actor>> Sources { get; init; } = sources;
...
public class PersistentInvertibleVoidzone(BossModule module, float radius, Func<IEnumerable<Actor>> sources, ActionID aid = default) : CastCounter(module, aid)
{
    public AOEShapeCircle Shape { get; init; } = new(radius);
    public Func<IEnumerable<Actor>> Sources { get; init; } = sources;
    ...
    {
        var inVoidzone = Sources().Any(s => Shape.Check(actor.Position, s));
        if (Inverted)
...
public class PersistentInvertibleVoidzoneByCast(BossModule module, float radius, Func<IEnumerable<Actor>> sources, ActionID aid) : PersistentInvertibleVoidzone(module, radius, sources, aid)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action == WatchedAction)
