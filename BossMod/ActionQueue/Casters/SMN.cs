...
public sealed class Definitions : IDisposable
{
    ...
    private void Customize(ActionDefinitions d)
    {
        ...
        d.Spell(AID.CrimsonCyclone)!.ForbidExecute = ActionDefinitions.GapCloserDangerCheck;

        // can only be executed when current pet is carby
        // pet glamours change nameID and appearance, but not OID
        d.Spell(AID.RadiantAegis)!.ForbidExecute = (ws, player, target, hints) => ws.Actors.Find(ws.Client.ActivePet.InstanceID)?.OID != 0x34BA;
    }
}
