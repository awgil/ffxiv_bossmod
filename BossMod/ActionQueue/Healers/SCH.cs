...
public sealed class Definitions : IDisposable
{
    ...
    private void Customize(ActionDefinitions d)
    {
        ...
        d.Spell(AID.Excogitation)!.ForbidExecute =
            (ws, player, _, _) => ws.Client.GetGauge<ScholarGauge>().Aetherflow == 0 && player.FindStatus(SID.Recitation) == null;

        d.Spell(AID.SacredSoil)!.TransformPosition = (ws, player, target, hints) => _config.Position switch
        {
            SCHConfig.SoilOverride.Self => player.PosRot.XYZ(),
            SCHConfig.SoilOverride.Fairy => (ws.Actors.Find(ws.Client.ActivePet.InstanceID) ?? player).PosRot.XYZ(),
            _ => null
        };
