...
public sealed class ZoneModuleInfoAttribute(BossModuleInfo.Maturity maturity, uint cfcId) : Attribute
{
...
public static class ZoneModuleRegistry
{
    ...
    static ZoneModuleRegistry()
    {
        foreach (var t in Utils.GetDerivedTypes<ZoneModule>(Assembly.GetExecutingAssembly()).Where(t => !t.IsAbstract))
        {
            ...
            {
                Service.Log($"Two zone modules have same CFCID: {t.Name} and {_modulesByCFC[attr.CFCID].ModuleType.Name}");
                continue;
            }
