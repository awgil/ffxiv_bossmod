...
public abstract record class AOEShape
{
    ...
    public virtual Func<WPos, float> Distance(WPos origin, Angle rotation) => throw new NotImplementedException("Please implement either this function or GetSdf");
    public virtual Sdf GetSdf(WPos origin, Angle rotation) => Sdf.Continuous(Distance(origin, rotation));
...
public sealed record class AOEShapeCustom(RelSimplifiedComplexPolygon Poly, Angle DirectionOffset = default) : AOEShape
{
    ...
    public override Sdf GetSdf(WPos origin, Angle rotation) => Sdf.Discrete(p => Check(p, origin, rotation));
}
