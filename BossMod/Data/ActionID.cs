...
public readonly record struct ActionID(uint Raw)
{
    ...
    {
        ActionType.Spell => Service.LuminaRow<Lumina.Excel.Sheets.Action>(ID)?.Name ?? "<not found>",
    ...
    {
        ActionType.Spell => ID,
        ActionType.Item => Service.LuminaRow<Lumina.Excel.Sheets.Item>(ID % 500000)?.ItemAction.Value?.Type ?? 0,
        ActionType.KeyItem => Service.LuminaRow<Lumina.Excel.Sheets.EventItem>(ID)?.Action.Row ?? 0,
        ActionType.Ability => 2, // 'interaction'
        ActionType.General => Service.LuminaRow<Lumina.Excel.Sheets.GeneralAction>(ID)?.Action.Row ?? 0, // note: duty action 1/2 (26/27) use special code
    ...
    public readonly float CastTimeExtra() => Service.LuminaRow<Lumina.Excel.Sheets.Action>(SpellId())?.Unknown38 * 0.1f ?? 0;
