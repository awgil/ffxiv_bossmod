...
public sealed class DRG(RotationModuleManager manager, Actor player) : Attackxan<AID, TraitID>(manager, player, PotionType.Strength)
{
    ...
    {
        var def = new RotationModuleDefinition("xan DRG", "Dragoon", "Standard Rotations|xan|Melee", "xan", RotationModuleQuality.Basic, BitMask.Build(Class.DRG, Class.LNC), 100);
