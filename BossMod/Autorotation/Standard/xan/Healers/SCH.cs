...
public sealed class SCH(RotationModuleManager manager, Actor player) : Castxan<AID, TraitID>(manager, player, PotionType.Mind)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        def.Define(Track.Place).As<FairyPlacement>("FairyPlace", "Fairy placement")
            .AddOption(FairyPlacement.Manual, "Manual", "Do not automatically move fairy")
            .AddOption(FairyPlacement.AutoHeel, "AutoHeel", "Automatically order fairy to follow player when combat ends")
            .AddOption(FairyPlacement.FullAuto, "FullAuto", "Automatically place fairy at arena center (if one exists) - order fairy to follow when out of combat");
