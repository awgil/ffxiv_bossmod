...
public sealed class AkechiGNB(RotationModuleManager manager, Actor player) : AkechiTools<AID, TraitID>(manager, player)
{
    ...
    public static RotationModuleDefinition Definition()
    {
        ...
        res.Define(Track.Combo).As<ComboStrategy>("Combo", uiPriority: 200)
            .AddOption(ComboStrategy.ForceSTwithO, "ForceSTwithO", "if 'Force Single-Target Rotation' is selected, it will prevent overcapping cartridges with Burst Strike (if available)")
            .AddOption(ComboStrategy.ForceSTwithoutO, "ForceSTwithoutO", "if 'Force Single-Target Rotation' is selected, it will not prevent overcapping cartridges")
            .AddOption(ComboStrategy.ForceAOEwithO, "ForceAOEwithO", "if 'Force AOE Rotation' is selected, it will prevent overcapping cartridges with Fated Circle (if available)")
            .AddOption(ComboStrategy.ForceAOEwithoutO, "ForceAOEwithoutO", "if 'Force AOE Rotation' is selected, it will not prevent overcapping cartridges");
        res.Define(Track.Cartridges).As<CartridgeStrategy>("Cartridges", "Carts", uiPriority: 180)
            ...
            .AddOption(CartridgeStrategy.OnlyBS, "OnlyBS", "Uses Burst Strike optimally as cartridge spender only, regardless of targets", supportedTargets: ActionTargets.Hostile, minLevel: 30)
            .AddOption(CartridgeStrategy.OnlyFC, "OnlyFC", "Uses Fated Circle optimally as cartridge spender only, regardless of targets", supportedTargets: ActionTargets.Hostile, minLevel: 72)
            .AddOption(CartridgeStrategy.ForceBS, "ForceBS", "Force use of Burst Strike regardless of cartridge count", 0, 0, ActionTargets.Hostile, 30)
            .AddOption(CartridgeStrategy.ForceBS1, "ForceBS1", "Force use of Burst Strike when only 1 cartridge is available", 0, 0, ActionTargets.Hostile, 30)
            .AddOption(CartridgeStrategy.ForceBS2, "ForceBS2", "Force use of Burst Strike when only 2 cartridges are available", 0, 0, ActionTargets.Hostile, 30)
            .AddOption(CartridgeStrategy.ForceBS3, "ForceBS3", "Force use of Burst Strike when only 3 cartridges are available", 0, 0, ActionTargets.Hostile, 30)
            .AddOption(CartridgeStrategy.ForceFC, "ForceFC", "Force use of Fated Circle when any cartridges are available", 0, 0, ActionTargets.Self, 72)
            .AddOption(CartridgeStrategy.ForceFC1, "ForceFC1", "Force use of Fated Circle when only 1 cartridge is available", 0, 0, ActionTargets.Self, 72)
            .AddOption(CartridgeStrategy.ForceFC2, "ForceFC2", "Force use of Fated Circle when only 2 cartridges are available", 0, 0, ActionTargets.Self, 72)
            .AddOption(CartridgeStrategy.ForceFC3, "ForceFC3", "Force use of Fated Circle when only 3 cartridges are available", 0, 0, ActionTargets.Self, 72)
        ...
        res.Define(Track.LightningShot).As<LightningShotStrategy>("Lightning Shot", "L.Shot", uiPriority: 100)
            .AddOption(LightningShotStrategy.OpenerFar, "OpenerFar", "Use Lightning Shot in pre-pull & out of melee range", supportedTargets: ActionTargets.Hostile)
            .AddOption(LightningShotStrategy.OpenerForce, "OpenerForce", "Force use Lightning Shot in pre-pull in any range", supportedTargets: ActionTargets.Hostile)
        ...
        res.Define(Track.NoMercy).As<NoMercyStrategy>("No Mercy", "N.Mercy", uiPriority: 160)
            .AddOption(NoMercyStrategy.Automatic, "Automatic", "Normal use of No Mercy", supportedTargets: ActionTargets.Self)
            .AddOption(NoMercyStrategy.BurstReady, "BurstReady", "Use No Mercy only when burst is ready; will delay if necessary", supportedTargets: ActionTargets.Self)
            .AddOption(NoMercyStrategy.Force, "Force", "Force use of No Mercy, regardless of weaving", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.ForceW, "ForceW", "Force use of No Mercy in next possible weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.ForceQW, "ForceQW", "Force use of No Mercy in next possible last second weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force1, "Force1", "Force use of No Mercy when only 1 cartridge is available, regardless of weaving", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force1W, "Force1W", "Force use of No Mercy when only 1 cartridge is available & in next weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force1QW, "Force1QW", "Force use of No Mercy when only 1 cartridge is available & in next possible last-second weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force2, "Force2", "Force use of No Mercy when only 2 cartridges are available, regardless of weaving", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force2W, "Force2W", "Force use of No Mercy when only 2 cartridges are available & in next possible weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force2QW, "Force2QW", "Force use of No Mercy when only 2 cartridges are available & in next possible last-second weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force3, "Force3", "Force use of No Mercy when only 3 cartridges are available, regardless of weaving", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force3W, "Force3W", "Force use of No Mercy when only 3 cartridges are available & in next possible weave slot", 60, 20, ActionTargets.Self, 2)
            .AddOption(NoMercyStrategy.Force3QW, "Force3QW", "Force use of No Mercy when only 3 cartridges are available & in next possible last-second weave slot", 60, 20, ActionTargets.Self, 2)
        ...
        res.Define(Track.SonicBreak).As<SonicBreakStrategy>("Sonic Break", "S.Break", uiPriority: 145)
            .AddOption(SonicBreakStrategy.Automatic, "Automatic", "Normal use of Sonic Break", supportedTargets: ActionTargets.Hostile, minLevel: 54)
            .AddOption(SonicBreakStrategy.Early, "Early", "Uses Sonic Break as the very first GCD when in No Mercy", supportedTargets: ActionTargets.Hostile, minLevel: 54)
            .AddOption(SonicBreakStrategy.Late, "Late", "Uses Sonic Break as the very last GCD when in No Mercy", supportedTargets: ActionTargets.Hostile, minLevel: 54)
        ...
        res.Define(Track.GnashingFang).As<GnashingStrategy>("Gnashing Fang", "G.Fang", uiPriority: 150)
            .AddOption(GnashingStrategy.Automatic, "Automatic", "Normal use of Gnashing Fang", supportedTargets: ActionTargets.Hostile, minLevel: 60)
            .AddOption(GnashingStrategy.ForceGnash, "ForceGnash", "Force use of Gnashing Fang", 30, 0, ActionTargets.Hostile, 60)
            .AddOption(GnashingStrategy.ForceGnash1, "ForceGnash1", "Force use of Gnashing Fang when only 1 cartridge is available", 30, 0, ActionTargets.Hostile, 60)
            .AddOption(GnashingStrategy.ForceGnash2, "ForceGnash2", "Force use of Gnashing Fang when only 2 cartridges are available", 30, 0, ActionTargets.Hostile, 60)
            .AddOption(GnashingStrategy.ForceGnash3, "ForceGnash3", "Force use of Gnashing Fang when only 3 cartridges are available", 30, 0, ActionTargets.Hostile, 60)
            .AddOption(GnashingStrategy.ForceClaw, "ForceClaw", "Force use of Savage Claw", 0, 0, ActionTargets.Hostile, 60)
            .AddOption(GnashingStrategy.ForceTalon, "ForceTalon", "Force use of Wicked Talon", 0, 0, ActionTargets.Hostile, 60)
        ...
        res.Define(Track.Continuation).As<ContinuationStrategy>("Continuation", "Cont.", uiPriority: 135)
            .AddOption(ContinuationStrategy.Automatic, "Automatic", "Normal use of Continuation", supportedTargets: ActionTargets.Hostile, minLevel: 70)
        ...
        res.Define(Track.Bloodfest).As<BloodfestStrategy>("Bloodfest", "Fest", uiPriority: 150)
            .AddOption(BloodfestStrategy.Automatic, "Automatic", "Normal use of Bloodfest", supportedTargets: ActionTargets.Hostile, minLevel: 80)
            ...
            .AddOption(BloodfestStrategy.ForceW, "ForceW", "Force use of Bloodfest in next possible weave slot, regardless of ammo count", 120, 0, ActionTargets.Hostile, 80)
            .AddOption(BloodfestStrategy.Force0, "Force0", "Force use of Bloodfest only if empty on cartridges", 120, 0, ActionTargets.Hostile, 80)
            .AddOption(BloodfestStrategy.Force0W, "Force0W", "Force use of Bloodfest only if empty on cartridges & in next possible weave slot", 120, 0, ActionTargets.Hostile, 80)
        ...
        res.Define(Track.DoubleDown).As<DoubleDownStrategy>("DoubleDown", "D.Down", uiPriority: 130)
            ...
            .AddOption(DoubleDownStrategy.Force, "Force", "Force use of Double Down regardless of cartridge count", 60, 0, ActionTargets.Self, 90)
            .AddOption(DoubleDownStrategy.Force1, "Force1", "Force use of Double Down when only 1 cartridge is available", 60, 0, ActionTargets.Self, 90)
            .AddOption(DoubleDownStrategy.Force2, "Force2", "Force use of Double Down when only 2 cartridges are available", 60, 0, ActionTargets.Self, 90)
            .AddOption(DoubleDownStrategy.Force3, "Force3", "Force use of Double Down when only 3 cartridges are available", 60, 0, ActionTargets.Self, 90)
        ...
        res.Define(Track.Reign).As<ReignStrategy>("Reign of Beasts", "Reign", uiPriority: 125)
            .AddOption(ReignStrategy.Automatic, "Automatic", "Normal use of Reign of Beasts", supportedTargets: ActionTargets.Hostile, minLevel: 100)
            .AddOption(ReignStrategy.ForceReign, "ForceReign", "Force use of Reign of Beasts", supportedTargets: ActionTargets.Hostile, minLevel: 100)
            .AddOption(ReignStrategy.ForceNoble, "ForceNoble", "Force use of Noble Blood", supportedTargets: ActionTargets.Hostile, minLevel: 100)
            .AddOption(ReignStrategy.ForceLion, "ForceLion", "Force use of Lion Heart", supportedTargets: ActionTargets.Hostile, minLevel: 100)
