...
static class ModuleExtensions
{
    ...
    public static RotationModuleDefinition.ConfigRef<AOEStrategy> DefineAOE(this RotationModuleDefinition res)
    {
        return res.Define(SharedTrack.AOE).As<AOEStrategy>("AOE", uiPriority: 300)
            .AddOption(AOEStrategy.AutoFinish, "AutoFinish", "Automatically execute optimal rotation based on targets; finishes combo if possible", supportedTargets: ActionTargets.Hostile)
            .AddOption(AOEStrategy.AutoBreak, "AutoBreak", "Automatically execute optimal rotation based on targets; breaks combo if necessary", supportedTargets: ActionTargets.Hostile)
    ...
    public static RotationModuleDefinition.ConfigRef<HoldStrategy> DefineHold(this RotationModuleDefinition res)
    {
        return res.Define(SharedTrack.Hold).As<HoldStrategy>("Hold", uiPriority: 290)
            ...
            .AddOption(HoldStrategy.HoldCooldowns, "HoldCooldowns", "Forbid use of all cooldowns only")
    ...
    public static RotationModuleDefinition.ConfigRef<PotionStrategy> DefinePotion(this RotationModuleDefinition res, ActionID pot)
    {
        return res.Define(SharedTrack.Potion).As<PotionStrategy>("Potion", uiPriority: 280)
            ...
            .AddOption(PotionStrategy.AlignWithBuffs, "AlignWithBuffs", "Use potion when personal buffs are imminent or active")
            .AddOption(PotionStrategy.AlignWithRaidBuffs, "AlignWithRaidBuffs", "Use potion when party raid buffs are imminent or active")
        ...
        where AID : Enum
    {
        ...
        return res.Define(track).As<GCDStrategy>(internalName, displayName: displayName, uiPriority: uiPriority)
            .AddOption(GCDStrategy.Automatic, "Automatic", "Auto", $"Automatically use {action.Name()} when optimal", cooldown, effectDuration, supportedTargets, minLevel, maxLevel)
            .AddOption(GCDStrategy.RaidBuffsOnly, "RaidBuffsOnly", "With Buffs", $"Use {action.Name()} when raid buffs are active", cooldown, effectDuration, supportedTargets, minLevel, maxLevel)
        ...
        where AID : Enum
    {
        ...
        return res.Define(track).As<OGCDStrategy>(internalName, displayName: displayName, uiPriority: uiPriority)
            .AddOption(OGCDStrategy.Automatic, "Automatic", "Auto", $"Automatically use {action.Name()} when optimal", cooldown, effectDuration, supportedTargets, minLevel: minLevel, maxLevel)
            .AddOption(OGCDStrategy.RaidBuffsOnly, "RaidBuffsOnly", "With Buffs", $"Use {action.Name()} when raid buffs are active", cooldown, effectDuration, supportedTargets, minLevel: minLevel, maxLevel)
