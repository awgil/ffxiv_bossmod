...
public sealed class AkechiWHMPvP(RotationModuleManager manager, Actor player) : AkechiTools<AID, TraitID>(manager, player)
{
    ...
    public override void Execution(StrategyValues strategy, Enemy? primaryTarget)
    {
        ...
        if (HasLOS(primaryTarget?.Actor))
        {
            ...
            if (role switch
            {
                RoleActionStrategy.Haelan => HasEffect(SID.HaelanEquippedPvP) && MP >= 2500 && PlayerHPP < 60,
                RoleActionStrategy.Stoneskin2 => HasEffect(SID.StoneskinEquippedPvP) && IsReady(AID.StoneskinIIPvP) && EnemiesTargetingSelf(2),
                RoleActionStrategy.Diabrosis => HasEffect(SID.DiabrosisEquippedPvP) && IsReady(AID.DiabrosisPvP),
                _ => false
            })
                QueueGCD(role switch
                {
                    RoleActionStrategy.Haelan => AID.HaelanPvP,
                    RoleActionStrategy.Stoneskin2 => AID.StoneskinIIPvP,
                    RoleActionStrategy.Diabrosis => AID.DiabrosisPvP,
                    _ => AID.None
                },
                role switch
                {
                    RoleActionStrategy.Haelan => Player,
