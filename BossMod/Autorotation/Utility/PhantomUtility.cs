...
public class PhantomUtility(RotationModuleManager manager, Actor player) : GenericUtility(manager, player)
{
    ...
    public static RotationModuleDefinition Definition(ActionDefinitions defs)
    {
        ...
        DefineSimpleConfig(defs, def, Track.Guard, "PGuard", "Phantom Guard", -10, PhantomID.PhantomGuard, 10);
        DefineSimpleConfig(defs, def, Track.Pledge, "Pledge", "Pledge", -10, PhantomID.Pledge, 10);
        DefineSimpleConfig(defs, def, Track.Shirahadori, "Shirahadori", "Shira", -10, PhantomID.Shirahadori, 4);
        DefineSimpleConfig(defs, def, Track.Unicorn, "Unicorn", "Unicorn", -10, PhantomID.OccultUnicorn, 30);
        DefineSimpleConfig(defs, def, Track.Bell, "BattleBell", "Bell", -10, PhantomID.BattleBell, 60);
        DefineSimpleConfig(defs, def, Track.RomeosBallad, "Romeo", "Romeo", -10, PhantomID.RomeosBallad, 3);
        DefineSimpleConfig(defs, def, Track.MightyMarch, "MightyMarch", "March", -10, PhantomID.MightyMarch, 30);
        DefineSimpleConfig(defs, def, Track.Predict, "Predict", "Predict", -10, PhantomID.Predict, 16, ActionQueue.Priority.VeryHigh - 10);
        DefineSimpleConfig(defs, def, Track.Rejuvenation, "Rejuvenation", "Rejuv", -10, PhantomID.PhantomRejuvenation, 20);
        DefineSimpleConfig(defs, def, Track.Invuln, "Invulnerability", "Invuln", -10, PhantomID.Invulnerability, 8);
