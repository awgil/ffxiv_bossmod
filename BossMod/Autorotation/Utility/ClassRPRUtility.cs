...
public sealed class ClassRPRUtility(RotationModuleManager manager, Actor player) : RoleMeleeUtility(manager, player)
{
    ...
    {
        var res = new RotationModuleDefinition("Utility: RPR", "Planner support for utility actions", "xan, Akechi", RotationModuleQuality.Ok, BitMask.Build((int)Class.RPR), 100);
