...
public sealed class ClassBLUUtility(RotationModuleManager manager, Actor player) : RoleCasterUtility(manager, player)
{
    ...
    {
        var res = new RotationModuleDefinition("Utility: BLU", "Planner support for utility actions", "Akechi", RotationModuleQuality.Basic, BitMask.Build((int)Class.BLU), 80);
