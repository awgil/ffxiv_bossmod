...
public sealed class NormalMovement(RotationModuleManager manager, Actor player) : RotationModule(manager, player)
{
    ...
    {
        var res = new RotationModuleDefinition("Auto-Movement", "Automatically move character based on pathfinding or explicit coordinates", "AI|Auto", "veyn", RotationModuleQuality.Good, new(~0ul), 1000, 1, RotationModuleOrder.Movement, CanUseWhileRoleplaying: true);
