...
) : StrategyConfig(InternalName, DisplayName, UIPriority)
{
    ...
    public override bool DrawForSimpleEditor(ref StrategyValue currentValue)
    {
        ...
        if (UICombo.EnumIndex(UIName, OptionEnum, ref opt, (int ix) => Options[ix].DisplayName.Length > 0 ? Options[ix].DisplayName : UICombo.EnumString((Enum)OptionEnum.GetEnumValues().GetValue(ix)!)))
        {
