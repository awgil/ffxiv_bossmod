namespace BossMod.Dawntrail.Foray.ForkedTower.FTM4Index;
...
class Bounds(BossModule module) : BossComponent(module)
{
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            if (state == 0x00020001)
                Arena.Bounds = FTM4Index.MakeIndexBounds(true);
            if (state == 0x00080004)
                Arena.Bounds = FTM4Index.MakeIndexBounds(false);
        }
    }
}
...
class ElementaryEvocation(BossModule module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (element != default)
        {
            ...
            var closest = ((FTM4Index)Module).GetPlatforms(element).MinBy(p => p.DistanceToAngle(startingAngle));
...
class ElementaryExpansion(BossModule module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (e != default)
        {
            foreach (var a in ((FTM4Index)Module).GetPlatforms(e).Take(1))
