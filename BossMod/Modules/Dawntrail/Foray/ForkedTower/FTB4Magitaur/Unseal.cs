...
class UnsealAutos(BossModule module) : Components.CastCounterMulti(module, [AID.AutoAttackLance, AID.AutoAttackAxe])
{
    ...
    public override void Update()
    {
        ...
        foreach (var group in WorldState.Actors.Where(p => p.Type == ActorType.Player && !p.IsDead).GroupBy(FTB4Magitaur.GetPlatform))
        {
            if (group.Key < 0)
                continue;
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        {
            var platform = FTB4Magitaur.GetPlatform(actor);
            if (platform < 0)
