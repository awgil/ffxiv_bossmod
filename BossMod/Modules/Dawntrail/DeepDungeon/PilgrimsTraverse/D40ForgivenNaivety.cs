    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (srcs.Count > 0)
        {
            var safeCone = srcs.Count > 1
                ...
                ? ShapeDistance.InvertedCone(srcs[0].Origin, 3, (srcs[1].Origin - srcs[0].Origin).ToAngle(), 21.Degrees())
                // just aim inside arena, safe angle is ~51.32 degrees
                : ShapeDistance.InvertedCone(srcs[0].Origin, 3, srcs[0].Direction, 51.Degrees());
...
[ModuleInfo(GroupType = BossModuleInfo.GroupType.CFC, GroupID = 1035, NameID = 13977)]
