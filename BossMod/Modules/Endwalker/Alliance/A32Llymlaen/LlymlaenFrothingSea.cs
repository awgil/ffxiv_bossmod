...
public class FrothingSea : Components.Exaflare
{
    ...
    public override void OnEventEnvControl(BossModule module, byte index, uint state)
    {
        ...
        if (state == 0x00800040 && index == 0x49)
            Lines.Add(new() { Next = new(module.Bounds.Center.X - 80, module.Bounds.Center.Z), Advance = 2.2f * _rot1.ToDirection(), NextExplosion = _activation, TimeToMove = 0.9f, ExplosionsLeft = 13, MaxShownExplosions = 2, Rotation = _rot1 });
        if (state == 0x08000400 && index == 0x49)
            Lines.Add(new() { Next = new(module.Bounds.Center.X + 80, module.Bounds.Center.Z), Advance = 2.2f * _rot2.ToDirection(), NextExplosion = _activation, TimeToMove = 0.9f, ExplosionsLeft = 13, MaxShownExplosions = 2, Rotation = _rot2 });
    }
