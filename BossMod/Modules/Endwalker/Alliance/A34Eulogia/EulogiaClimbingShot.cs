...
class ClimbingShot(BossModule module) : Components.Knockback(module)
{
    ...
    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos) => Module.FindComponent<AsAboveSoBelow>()?.ActiveAOEs(slot, actor).Any(z => z.Shape.Check(pos, z.Origin, z.Rotation)) ?? false || !Module.Bounds.Contains(pos);
}
