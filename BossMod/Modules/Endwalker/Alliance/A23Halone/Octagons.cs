...
class Octagons(BossModule module) : BossComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        else if (Module.Enemies(OID.GlacialSpearSmall).Any(x => x.Position.AlmostEqual(spears[0], 1) && !x.IsDead) && actor.Position.InPolygon(Octagon1()))
            octagons.Add(ShapeDistance.InvertedConvexPolygon(Octagon1(), true));
        else if (Module.Enemies(OID.GlacialSpearSmall).Any(x => x.Position.AlmostEqual(spears[1], 1) && !x.IsDead) && actor.Position.InPolygon(Octagon2()))
            octagons.Add(ShapeDistance.InvertedConvexPolygon(Octagon2(), true));
        else if (Module.Enemies(OID.GlacialSpearSmall).Any(x => x.Position.AlmostEqual(spears[2], 1) && !x.IsDead) && actor.Position.InPolygon(Octagon3()))
            octagons.Add(ShapeDistance.InvertedConvexPolygon(Octagon3(), true));
        if (octagons.Count == 0)
        {
            if (Module.Enemies(OID.GlacialSpearSmall).Any(x => x.Position.AlmostEqual(spears[0], 1) && !x.IsDead) && !actor.Position.InPolygon(Octagon1()))
                octagons2.Add(ShapeDistance.ConvexPolygon(Octagon1(), true));
            if (Module.Enemies(OID.GlacialSpearSmall).Any(x => x.Position.AlmostEqual(spears[1], 1) && !x.IsDead) && !actor.Position.InPolygon(Octagon2()))
                octagons2.Add(ShapeDistance.ConvexPolygon(Octagon2(), true));
            if (Module.Enemies(OID.GlacialSpearSmall).Any(x => x.Position.AlmostEqual(spears[2], 1) && !x.IsDead) && !actor.Position.InPolygon(Octagon3()))
