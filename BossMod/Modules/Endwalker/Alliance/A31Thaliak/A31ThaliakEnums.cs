...
public enum AID : uint
{
    ...
    Katarraktes = 35025, // Thaliak->self, 5.0s cast, single-target, visual (raidwide)
    KatarraktesAOE = 35034, // Helper->self, 5.7s cast, range 70 circle, raidwide
    Thlipsis = 35032, // Thaliak->self, 4.0s cast, single-target, visual (stack)
    ThlipsisAOE = 35033, // Helper->players, 6.0s cast, range 6 circle stack
    Hydroptosis = 35028, // Thaliak->self, 4.0s cast, single-target, visual (spread)
    HydroptosisAOE = 35029, // Helper->player, 5.0s cast, range 6 circle spread
    Rhyton = 35030, // Thaliak->self, 5.0s cast, single-target, visual (baited line tankbusters)
    ...
    Rheognosis = 35012, // Thaliak->self, 5.0s cast, single-target, visual (knockback across arena)
    RheognosisPetrine = 35013, // Thaliak->self, 5.0s cast, single-target, visual (knockback across arena + aoes)
    ...
    Tetraktys = 35017, // Thaliak->self, 6.0s cast, single-target, visual (triangles)
    TetraktysAOESmall = 35018, // Helper->self, 1.8s cast, triangle 16
    TetraktysAOELarge = 35019, // Helper->self, 1.8s cast, triangle 32
    TetraktuosKosmos = 35020, // Thaliak->self, 4.0s cast, single-target, visual (splitting triangle)
    ...
    Hieroglyphika = 35023, // Thaliak->self, 5.0s cast, single-target, visual (rotating safespots)
    HieroglyphikaAOE = 35024, // Helper->self, 3.0s cast, range 12 width 12 rect
    HieroglyphikaLeftBank = 35884, // Thaliak->self, 22.0s cast, range 60 180-degree cone
    HieroglyphikaRightBank = 35885, // Thaliak->self, 22.0s cast, range 60 180-degree cone
}
