...
class ClimbingShot(BossModule module) : Components.Knockback(module)
{
    ...
    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos) => Module.FindComponent<DownhillP1>()?.ActiveAOEs(slot, actor).Any(z => z.Shape.Check(pos, z.Origin, z.Rotation)) ?? false || !Module.InBounds(pos);
}
