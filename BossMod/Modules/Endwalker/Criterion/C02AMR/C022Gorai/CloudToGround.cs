...
class CloudToGround(BossModule module) : Components.Exaflare(module, 6)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if ((AID)spell.Action.ID is AID.NCloudToGroundAOEFirst or AID.SCloudToGroundAOEFirst)
        {
            ...
            Lines.Add(new() { Next = caster.Position, Advance = 6 * spell.Rotation.ToDirection(), NextExplosion = spell.NPCFinishAt, TimeToMove = 1.1f, ExplosionsLeft = (caster.Position - Module.Center).LengthSq() > 100 ? 7 : 3, MaxShownExplosions = 3 });
        }
    }
