namespace BossMod.Endwalker.Savage.P5SProtoCarbuncle
{
    class P5SStates : StateMachineBuilder
    {
        ...
        private void SinglePhase(uint id)
        {
            ...
            VenomousMassToxicCrunch(id + 0xE0000, 9.3f);
            ...
            VenomousMassToxicCrunch(id + 0x140000, 3.2f);
        ...
        private void RubyGlowTopazStones(uint id, float delay)
        {
            ...
            ComponentCondition<TopazStones>(id + 0x1100, 14.1f, comp => comp.NumCasts > 0, "Cells")
        ...
        private void RubyGlowTopazStonesDoubleRush(uint id, float delay)
        {
            ...
            // TODO: show poison, show swap/stay hint before first charge, show cell aoe after first charge
            RubyGlow(id, delay);
            Cast(id + 0x1000, AID.TopazStones, 3.2f, 4);
            Cast(id + 0x1010, AID.DoubleRush, 4.5f, 6, "Charge 1")
                .ActivateOnEnter<DoubleRush>()
                .DeactivateOnExit<DoubleRush>();
            ComponentCondition<DoubleRushReturn>(id + 0x1012, 2.1f, comp => comp.NumCasts > 0, "Charge 2")
                .ActivateOnEnter<DoubleRushReturn>()
                .DeactivateOnExit<DoubleRushReturn>();
            ComponentCondition<RubyReflection2>(id + 0x1020, 1.5f, comp => comp.NumCasts > 0, "Cells")
        ...
        private void RubyGlowTopazCluster(uint id, float delay)
        {
            ...
            ComponentCondition<TopazCluster>(id + 0x1010, 11.1f, comp => comp.NumCasts >= 2, "Cells 1");
        ...
        private void RubyGlowTopazStonesVenomPoolRayClaw(uint id, float delay)
        {
            ...
            // +3.9s: venom pool aoes
            // +4.4s: recolor
            // +6.4s: magic visual cast end
            // +7.0s: magic explode (ruby reflection 2)
            CastMulti(id + 0x2000, new AID[] { AID.SearingRay, AID.RagingClaw }, 7, 5, "Searing ray / Raging claw");
        ...
        private void RubyGlowTopazStonesVenomSquall(uint id, float delay)
        {
            ...
            VenomSquallSurge(id + 0x2000, 9.1f, true);
        }
        ...
        private void RubyGlowTopazStonesVenomPoolDoubleRush(uint id, float delay)
        {
            ...
            // +6.4s: magic visual cast end
            // +7.0s: magic explode (ruby reflection 2)
            Cast(id + 0x2000, AID.DoubleRush, 10.9f, 6, "Charge 1")
                .ActivateOnEnter<DoubleRush>()
                .DeactivateOnExit<DoubleRush>();
            ComponentCondition<DoubleRushReturn>(id + 0x2002, 2.1f, comp => comp.NumCasts > 0, "Charge 2")
                .ActivateOnEnter<DoubleRushReturn>()
                .DeactivateOnExit<DoubleRushReturn>();
        }

        private void VenomSquallSurge(uint id, float delay, bool partOfRubyGlow = false)
        {
            CastMulti(id, new AID[] { AID.VenomSquall, AID.VenomSurge }, delay, 5, partOfRubyGlow ? "Cells" : "")
                .ActivateOnEnter<VenomSquallSurge>();
            ComponentCondition<VenomSquallSurge>(id + 2, 3.8f, comp => comp.Progress > 0, partOfRubyGlow ? "Spread" : "Spread/stack");
            ...
            ComponentCondition<VenomSquallSurge>(id + 5, 3, comp => comp.Progress > 2, partOfRubyGlow ? "Stack" : "Stack/spread")
