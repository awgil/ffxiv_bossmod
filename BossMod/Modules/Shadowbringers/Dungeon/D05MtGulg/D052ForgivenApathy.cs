...
public class D052ForgivenApathy : BossModule
{
    ...
    public D052ForgivenApathy(WorldState ws, Actor primary) : base(ws, primary, new ArenaBoundsCircle(default, default))
    {
    ...
    protected override void UpdateModule()
    {
        if (PrimaryActor.Position.AlmostEqual(new(-11, -193), 1))
            Arena.Bounds = new ArenaBoundsPolygon(arenacoords1);
        if (PrimaryActor.Position.AlmostEqual(new(-204, -106), 1))
            Arena.Bounds = new ArenaBoundsPolygon(arenacoords2);
    }
