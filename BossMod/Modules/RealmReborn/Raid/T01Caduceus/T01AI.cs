    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        foreach (var e in hints.PotentialTargets)
        {
            if ((OID)e.Actor.OID == OID.Boss)
            {
                if (e.Actor == Module.PrimaryActor)
                {
                    ...
                    if (cloneSpawningSoon && e.Actor.FindStatus(SID.SteelScales) != null)
                        e.Priority = AIHints.Enemy.PriorityUndesirable; // stop dps until stack can be dropped
                }
                else
                {
            ...
            else if ((OID)e.Actor.OID == OID.DarkMatterSlime)
            {
                ...
                e.Priority = predictedHP > 0.2f * e.Actor.HPMP.MaxHP ? (e.Actor.TargetID == actor.InstanceID ? 3 : AIHints.Enemy.PriorityUndesirable) : AIHints.Enemy.PriorityUndesirable;
