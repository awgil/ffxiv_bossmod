...
{
    class Editor(Bitmap initial, string storageDir) : UIBitmapEditor(initial)
    {
        protected override void DrawSidebar()
        {
            ...
            if (ImGui.Button("save"))
            {
                ...
                Bitmap.Save(Path.Join(storageDir, "test.bmp"));
            }

            if (ImGui.Button("load"))
            {
                using var stream = File.OpenRead(Path.Join(storageDir, "test.bmp"));
