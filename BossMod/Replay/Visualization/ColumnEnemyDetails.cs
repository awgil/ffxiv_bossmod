...
public class ColumnEnemyDetails : Timeline.ColumnGroup
{
    ...
    public ColumnEnemyDetails(BossModuleRegistry bmr, Timeline timeline, StateMachineTree tree, List<int> phaseBranches, Replay replay, Replay.Encounter enc, Replay.Participant enemy, ActionEffectParser aep)
        : base(timeline)
    {
        //Name = ReplayUtils.ParticipantString(enemy);
        _enemy = enemy;
        _casts = Add(new ColumnEnemyCasts(bmr, timeline, tree, phaseBranches, replay, enc, enemy, aep));
        _statuses = Add(new ColumnActorStatuses(timeline, tree, phaseBranches, replay, enc, enemy, aep));
        _hp = Add(new ColumnActorHP(timeline, tree, phaseBranches, replay, enc, enemy, aep));
