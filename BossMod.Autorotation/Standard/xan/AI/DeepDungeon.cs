...
public class DeepDungeonAI(RotationModuleManager manager, Actor player) : AIBase<DeepDungeonAI.Strategy>(manager, player)
{
    ...
    public override void Execute(in Strategy strategy, ref Actor? primaryTarget, float estimatedAnimLockDelay, bool isMoving)
    {
        ...
        if (IsRanged && !Player.InCombat && primaryTarget is Actor target && !target.InCombat && !target.IsAlly)
            ...
            Hints.GoalZones.Add(Hints.GoalSingleTarget(target, Player, World.Actors, 3, 0.1f));
    ...
    private void DoTransformActions(in Strategy strategy, Actor? primaryTarget, Transformation t)
    {
        ...
        switch (t)
        {
            case Transformation.Manticore:
                goal = Hints.GoalSingleTarget(primaryTarget, Player, World.Actors, 3);
                numTargets = 1;
                attack = ActionID.MakeSpell(Roleplay.AID.Pummel);
                break;
            case Transformation.Succubus:
                goal = Hints.GoalSingleTarget(primaryTarget, Player, World.Actors, 25);
            ...
            case Transformation.Bomb:
                numTargets = 1;
                goal = Hints.GoalSingleTarget(primaryTarget, Player, World.Actors, 14);
